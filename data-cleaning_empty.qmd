...
```
***Note***: `Water_cover` and `Land_cover` and `Total_cover` are currently character data types, we'd expect these values to be double.


***Is the data in long format?***: The data is in long format, since each row is representative of a column.

***Primary Key***: a way to identify a row/observation.
  - a primary key can be made up of multiple columns, making it compound primary key.
  
***What is the Primary Key of this database***: `Site`, `Plot`, `Location`, `Date`... possibly `Year`. Since `Year` seems to be repeated for each row/observation it may not be included in the compound primary key.
...
This tells us the unique characters and the amount of each character in the `Snow_cover` column.
...
***What kind of format is this data?***: The data is in a wide format.
  - we want year - numeric, site - character, date - date, species - character, controlled vocabulary, presence - binary (1/0)
